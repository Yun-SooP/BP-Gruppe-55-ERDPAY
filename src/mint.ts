...
export async function htmlMint(div: HTMLDivElement, session: Session) {
  ...
  if(document.querySelector<HTMLElement>(".tooltips") == null) {
    ...
    const text_mint = "Create your own cryptocurrency or NFTs through Minting. Each Token has an address and token ID. \n <br> You can assign any 40 digit hexadecimal to a new token or let us create a random address for you. \n<br> You can create one digital asset with a natural number token ID of your choice, or you can choose to create multiples with random token IDs.";
